...
class Luna {
  ...
  function __construct() {
    ...
    if (get_option(LUNA_SHORTNAME . 'login_errors_enabled')) {
  ...
  public function install() {
    ...
    // Define table names.
    $table_name_luna   = $wpdb->prefix . 'luna';
    
    // Check if the tables already exist.
    if ($wpdb->get_var("SHOW TABLES LIKE '" . $table_name_luna . "'") != $table_name_luna) {
      // Table SQL
      $table_luna = "CREATE TABLE IF NOT EXISTS `{$table_name_luna}` (
    ...
    if (!luna_option_exists('rows_to_display')) {
      add_option(LUNA_SHORTNAME . 'rows_to_display', '15');
    }
    // Enable by default the login errors override. If necessary this can be
    // changed from admin panel.
    if (!luna_option_exists('login_errors_enabled')) {
      add_option(LUNA_SHORTNAME . 'login_errors_enabled', TRUE);
    }
  ...
  public function login_errors() {
    $login_error_text = get_option(LUNA_SHORTNAME . 'login_error_text');
